...
func Register(c *gin.Context) {
	...
	if !utils.ValidatePseudo(req.Pseudo) {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Pseudo invalide (3-20 caracteres)."})
		return
	}
	...
	if count > 0 {
		c.JSON(http.StatusConflict, gin.H{"error": "Cet email est deja utilise."})
		return
	}
	database.ZUKUKDB.QueryRow(`SELECT COUNT(*) FROM users WHERE pseudo = ? AND deleted_at IS NULL`, req.Pseudo).Scan(&count)
	if count > 0 {
		c.JSON(http.StatusConflict, gin.H{"error": "Ce pseudo est deja pris."})
		return
	}
	...
	if _, err := database.ZUKUKDB.Exec(`
		...
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Erreur creation compte."})
		return
	}

	c.JSON(http.StatusCreated, gin.H{"message": "Compte cree.", "email": req.Email})
}
...
func Login(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Erreur creation session."})
		return
	}
	setSessionCookie(c, token)
	c.JSON(http.StatusOK, gin.H{"message": "Connecte."})
}
...
func GetMe(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		"id": userID, "pseudo": pseudo, "email": email, "avatar_url": avatarURL,
...
func UpdateProfile(c *gin.Context) {
	...
	if req.Pseudo != "" {
		...
		if _, err := database.ZUKUKDB.Exec(`UPDATE users SET pseudo = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, req.Pseudo, userID); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Erreur mise a jour pseudo."})
			return
		}
	}
	...
	if req.Email != "" {
		...
		if _, err := database.ZUKUKDB.Exec(`UPDATE users SET email = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, req.Email, userID); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Erreur mise a jour email."})
			return
		}
	}
	...
	if req.AvatarURL != "" {
		if _, err := database.ZUKUKDB.Exec(`UPDATE users SET avatar_url = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, strings.TrimSpace(req.AvatarURL), userID); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Erreur mise a jour avatar."})
			return
		}
	}
	...
	if req.Password != "" {
		...
		if _, err := database.ZUKUKDB.Exec(`UPDATE users SET password_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, string(hash), userID); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Erreur mise a jour mot de passe."})
			return
		}
	}

	c.JSON(http.StatusOK, gin.H{"message": "Profil mis a jour."})
}
...
func DeleteAccount(c *gin.Context) {
	...
	c.SetCookie("session_token", "", -1, "/", "", false, true)
	c.JSON(http.StatusOK, gin.H{"message": "Compte supprime."})
}
